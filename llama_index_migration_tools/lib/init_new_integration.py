...
def main(integration_type: str, integration_name: str, prefix: Optional[str] = None, base_file: Optional[str] = None):
    # create new directory, works in current directory
    pkg_name = (
        f"llama-index-{integration_type}-{integration_name}".replace(" ", "-").replace("_", "-").lower() if prefix is None else 
        f"llama-index-{prefix}-{integration_type}-{integration_name}".replace(" ", "-").replace("_", "-").lower()           
    )
    ...
    pkg_src_dir = os.path.join(
        pkg_path,
        (f"llama_index/{integration_type}/{integration_name}".replace(' ', '_').lower() if prefix is None else
        f"llama_index/{prefix}/{integration_type}/{integration_name}".replace(' ', '_').lower() 
         )
    )
    ...
    with open(pkg_src_dir + "/__init__.py", "w") as f:
        ...
        ) if prefix is None else init_with_prefix_str.format(
    ...
    test_file_name = tests_path + (f"/test_{integration_type}_{integration_name}.py".replace(" ", "_").lower() if prefix is None else 
            f"/test_{prefix}_{integration_type}_{integration_name}.py".replace(" ", "_").lower()
        )
